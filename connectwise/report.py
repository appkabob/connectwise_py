...
class UpcomingSchedule(Report):
    ...
    def _generate_report_content(self):
        ...
        titletext = "{} Report".format(self.name)
        ...
        if self.sort.lower() == 'consultant':
            ...
            member_identifiers = sorted(set([schedule_entry.member['identifier'] for schedule_entry in sorted_schedule_entries]))
            for member_identifier in member_identifiers:
                ...
                member_section.append(Paragraph('<strong>{}</strong>'.format(member_identifier), styles["Normal"]))
                member_section.append(Spacer(1, 4))
                member_section.append(t)
                Story.append(KeepTogether(member_section))

        elif self.sort.lower() == 'date':
            months = sorted(set([utc_to_local(datetime.strptime(schedule_entry.dateStart, '%Y-%m-%dT%H:%M:%SZ')).strftime('%Y-%-m-1')
            ...
            for month in months:
                ...
                for schedule_entry in sorted(self.schedule_entries, key=attrgetter('dateStart')):
                    ...
                    if month == utc_to_local(datetime.strptime(schedule_entry.dateStart, '%Y-%m-%dT%H:%M:%SZ')).strftime('%Y-%-m-1'):
                        month_table.append(
                            [Paragraph('{}<br />{}'.format(
                                utc_to_local(
                                    datetime.strptime(schedule_entry.dateStart, '%Y-%m-%dT%H:%M:%SZ')).strftime(
                ...
                print(themonth)
                Story.append(Paragraph('<strong>{}</strong>'.format(themonth), styles["Normal"]))
            ...
            company_names = sorted(set([schedule_entry.name.split(' / ')[0] for schedule_entry in sorted_schedule_entries]))
            for company_name in company_names:
                ...
                # Story.append(Paragraph('<strong>{}</strong>'.format(company_name), styles["Normal"]))
                company_section = []
                company_section.append(Spacer(1, 12))
                company_section.append(Paragraph('<strong>{}</strong>'.format(company_name), styles["Normal"]))
                company_section.append(Spacer(1, 4))
                company_section.append(t)
                Story.append(KeepTogether(company_section))
